...
const href = computed(() => {
  const num = (props.whatsapp.number || '').replace(/[^0-9]/g, '')
  const text = encodeURIComponent(props.whatsapp.defaultMessage || '')
...
<template>
  <a
    ...
    class="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-success text-white shadow-soft transition hover:scale-105"
  >
